...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('-a','--arch', nargs='+', metavar='ARCH', help='model architecture (default: fasterrcnn_resnet50_fpn)', default='fasterrcnn_resnet50_fpn')
...
def producer(loader, valid_loader, qs, qs_input, qs_labels, device, args, producer_alive):
    pid = os.getpid()
    if args.seed:
...
def worker(q, q_input, q_labels, model, args, producer_alive, finished_workers):
    ...
    while True:
        ...
        labels = [{'boxes': labels_slots[offset*12],
...
def main(args):
    ...
    for idx, arch in enumerate(args.arch):
        ...
        model = torchvision.models.get_model(
            arch, weights=args.weights, weights_backbone=args.weights_backbone, num_classes=num_classes, **kwargs
        )
        trainer = CocoTrainer(args, model, device, arch)
        if args.lr_scheduler == "multisteplr":
            epoch_lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(trainer.optimizer, milestones=args.lr_steps, gamma=args.lr_gamma)
        elif args.lr_scheduler == "cosineannealinglr":
            epoch_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(trainer.optimizer, T_max=args.epochs)
        else:
            raise RuntimeError(
